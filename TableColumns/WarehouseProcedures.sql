...
CREATE OR ALTER PROCEDURE dbo.CreateColumnAudit(@TableId NVARCHAR(128), @Name NVARCHAR(128),
...
CREATE OR ALTER PROCEDURE dbo.AlterColumnAudit(@Id NVARCHAR(128), @TableId NVARCHAR(128),
                                               @Name NVARCHAR(128), @Default NVARCHAR(MAX) = NULL,
                                               @DataTypeId NVARCHAR(128) = NULL, @NotNull BIT = 0)
AS
BEGIN
...
CREATE OR ALTER PROCEDURE dbo.DropColumnAudit(@Id NVARCHAR(128))
AS
BEGIN
    DECLARE @SchemaName VARCHAR(256), @TableId NVARCHAR(128), @TableName VARCHAR(256), @ColName VARCHAR(256);
    SELECT @SchemaName = CONCAT(S.Name, '_Audit'), @TableName = T.Name, @ColName = TC.Name
    FROM dbo.TableColumns AS TC
             INNER JOIN Tables AS T ON T.Id = TC.TableId
             INNER JOIN dbo.Schemas AS S ON S.Id = T.SchemaId
    WHERE TC.Id = @Id;
